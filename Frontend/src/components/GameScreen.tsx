...
export function GameScreen({ playerName, gameMode, lobbyPlayers, onGameOver }: Props) {
  ...
  const isMyTurnOver = me?.hasWon || me?.isEliminated || (myAttempts >= 5 && !state.isGameOver);
    ...
    <div className="min-h-screen flex flex-col">
      ...
      >
        <div>
          ...
          <p className="text-sm font-black">Wordle Wars</p>
        </div>

        {/* Score pill */}
        <div
          className="flex items-center gap-3 px-4 py-2 rounded-full text-sm font-black"
      ...
      <div className="flex-1 flex flex-col lg:flex-row gap-4 p-4 max-w-5xl mx-auto w-full">
        ...
        <div className="flex flex-col items-center gap-4 flex-1">
          ...
          {state.isGameOver && state.result && (
            <div
              className="w-full max-w-xs py-2 px-4 rounded-xl text-center text-sm font-black animate-pulse"
              style={{
          ...
          {me?.hasWon && !state.result && (
            <div
              className="w-full max-w-xs py-2 px-4 rounded-xl text-center text-sm font-black"
          ...
          {me?.isEliminated && !me.hasWon && !state.isGameOver && (
            <div
              className="w-full max-w-xs py-2 px-4 rounded-xl text-center text-sm font-semibold"
        ...
        <aside className="lg:w-64 xl:w-72">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-1 gap-2">
            ...
            {otherPlayers.length === 0 && (
              <p className="text-xs col-span-2 text-center py-4"
                 style={{ color: 'var(--color-muted-foreground)' }}>
                No other players
              </p>
            )}
                ...
                .map((p, i) => (
                  <div key={p.id} className="flex items-center justify-between py-1">
                    <div className="flex items-center gap-2">
                      <span className="text-xs w-4" style={{ color: 'var(--color-muted-foreground)' }}>
                        {i + 1}.
                      </span>
